...
/**
* A sequence of ThroughputCommandKind instances.
*/
public final class ThroughputCommandKindSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThroughputCommandKindSeq() {
        super(ThroughputCommandKind.class);
    }

    public ThroughputCommandKindSeq (int initialMaximum) {
        super(ThroughputCommandKind.class, initialMaximum);
    }

    public ThroughputCommandKindSeq (Collection elements) {
        super(ThroughputCommandKind.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThroughputCommandKind.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThroughputCommandKind objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThroughputCommandKind.create());
