...
#define DEFAULT_TOPIC_NAME          "Hello Dynamic"
...
static DDS_Boolean start_application(
    ...
    const char * const typeName = HelloWorldType_get_type_name();
    ...
    */
     ...
     */
    type = HelloWorldType_create();
    if (type == NULL) {
        fprintf(stderr, "! Unable to create dynamic type code\n");
        goto exitFn;
    }

    /* Create the Dynamic data type support object
     */
    typeSupport = DDS_DynamicDataTypeSupport_new(type, 
                        &props);
    if (typeSupport == NULL) {
    ...
    topic = DDS_DomainParticipant_create_topic(
                        ...
                        DDS_STATUS_MASK_NONE);
    if (topic == NULL) {
        fprintf(stderr, "! Unable to create topic '%s': %d\n", 
                        arg->topic_name,
                        rc);
        goto exitFn;
    }
...
int main(int argc, const char **argv) {
    ...
    for (i = 2; i < argc; ++i) {
        ...
        if (!RTI_STRNCMP(argv[i], "-s", 2) || 
            ...
            if (arg.data_size >= HELLODDS_MAX_PAYLOAD_SIZE) {
                usage();
                fprintf(stderr, "! Value too big for --size argument: %d\n",
                        arg.data_size);
                fprintf(stderr, "! In this example the max size is set "
