    ...
    PrimeNumberRequest * request)
{
    ...
    if (requester != NULL) {
        ...
        if (retcode != DDS_RETCODE_OK) {
            printf("delete requester error %d\n", retcode);
            status = -1;
        }
    }
...
int requester_main(int n, int primes_per_reply, int domain_id)
{
    ...
    if (retcode != DDS_RETCODE_OK) {
        if (retcode == DDS_RETCODE_TIMEOUT) {
            printf("Timed out waiting for prime numbers\n");
            return -1;
        } else {
            printf("Error receiving replies %d\n", retcode);
            return -1;
        }
    }
