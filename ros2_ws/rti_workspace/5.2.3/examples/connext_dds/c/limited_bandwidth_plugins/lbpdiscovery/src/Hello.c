...
#define DEFAULT_TOPIC_NAME          "Hello LBPDiscovery"
...
/* Qos profile information used to create the participant */
#define QOS_LIBRARY				"LBPDiscoveryPluginExample_Library"
#define PUBLISHER_PROFILE		"LBPDiscoveryPluginExamplePublisher_Profile"
#define SUBSCRIBER_PROFILE		"LBPDiscoveryPluginExampleSubscriber_Profile"
...
static DDS_Boolean start_application(
    ...
    */
     ...
     *
    {
        ...
        DDS_DomainParticipantFactory_get_default_participant_qos(DDS_TheParticipantFactory, 
                        &myQos);
    ...
    if (arg->mode == APP_MODE_PUBLISHER) {
		participant = DDS_DomainParticipantFactory_create_participant_with_profile(
			DDS_TheParticipantFactory, arg->domain_id, QOS_LIBRARY,
			PUBLISHER_PROFILE,
        ...
        if (!RTI_STRNCMP(argv[i], "-d", 2) || 
            ...
            if (arg.domain_id < 0 || arg.domain_id > DOMAIN_ID_MAX) {
                ...
                fprintf(stderr, "! Invalid DDS Domain ID: %d\n", 
                        arg.domain_id);
                fprintf(stderr, "! The domain ID must be between 0 "
                        "and %d (inclusive)\n", DOMAIN_ID_MAX);
                return EXIT_FAILURE;
            }
            continue;
