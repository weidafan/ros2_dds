...
static bool startApplication(
    ...
    DDSDynamicDataTypeSupport *typeSupport = NULL;
    ...
    */
    ...
    if (type == NULL) {
        std::cerr << "! Unable to create dynamic type code" << std::endl;
        goto exitFn;
    }

    /* Create the Dynamic data type support object
     */
    typeSupport = new DDSDynamicDataTypeSupport(type, 
                        props);
    if (typeSupport == NULL) {
        std::cerr << "! Unable to create dynamic data type support" 
                        << std::endl;
        goto exitFn;
    }

    /* Register type before creating topic */
    rc = typeSupport->register_type(participant, typeName);
    if (rc != DDS_RETCODE_OK) {
