...
class CommandLineArguments {
    ...
    static final String DEFAULT_TOPIC_NAME = "Hello Dist Logger - Log4j";
...
public class Hello {
    ...
    private static final org.apache.log4j.Logger logger = 
            org.apache.log4j.Logger.getLogger(Hello.class.toString());
    ...
    private static void startApplication(final CommandLineArguments arg) {
        ...
        if (!new File("USER_QOS_PROFILES.xml").exists()) {
            ...
            if (version.startsWith("1.4")) { /* deprecated System.getenv() */
                logger.warn("! Warning:");
                logger.warn("! Java "+version+" cannot determine if the"); 
                logger.warn("! environment variable NDDS_QOS_PROFILES is set.");
                logger.warn("! If not set, the application will use the ");
                logger.warn("! DDS default QoS.");
                logger.warn("! If you want to use different QoS, make sure you have the " +
                                   "QoS definition file");
                logger.warn("! (USER_QOS_PROFILES.xml) in the current working directory");
                logger.warn("! or set the environment variable NDDS_QOS_PROFILES to");
                logger.warn("! point to a file containing the default QoS profile");
            } else {
                String envVal = System.getenv("NDDS_QOS_PROFILES");
                if (envVal == null || new File(envVal).exists()) {
                    logger.warn("! Warning:");
                    logger.warn("! Default QoS profile definition file not found.");
                    logger.warn("! The application will use the DDS default QoS.");
                    logger.warn("! If you want to use different QoS, make sure you have the " +
                                "QoS definition file");
                    logger.warn("! (USER_QOS_PROFILES.xml) in the current working directory");
                    logger.warn("! or set the environment variable NDDS_QOS_PROFILES to");
        ...
        try {
            ...
            logger.error("An error occurred while changing factory QoS: " 
                        + e);
        }
        */
        ...
        Topic topic = participant.create_topic(
                        ...
                        StatusKind.STATUS_MASK_NONE);
        if (topic == null) {
            logger.error("! Unable to create topic " + arg.topicName);
            System.exit(-1);
        }           
    ...
    static boolean ensureOneMoreArgument(int i, String [] args, String argName) {
        if (i+1 > args.length) {
            usage();
            logger.error("! Error: missing value for " + argName +
                        " argument");
            return false;
        }
        return true;
    ...
    public static void main(String[] argv) {
        ...
        if (argv[0].equalsIgnoreCase("pub")) {
            ...
            usage();
            logger.error("! Invalid mode: " + argv[0]);
            logger.error("! Valid modes are only 'pub' or 'sub'.");
            System.exit(-1);
        }
        ...
        for (int i = 1; i < argv.length; ++i) { 
            ...
            if (argv[i].equals("-d") || argv[i].equals("--domain")) {
                ...
                if (args.domainId < 0 || 
                    ...
                    usage();
                    logger.error("! Invalid DDS Domain ID:" +
                        Integer.toString(args.domainId));
                    logger.error("! The domain ID must be between 0 and " +
            ...
            if (argv[i].equals("-s") || argv[i].equals("--size")) {
                ...
                if (args.dataSize <= 0) {
                    usage();
                    logger.error("! Invalid value for --size argument: " +
            ...
            if (argv[i].equals("-c") || argv[i].equals("--sampleCount")) {
                ...
                if (args.sampleCount < 0) {
                    usage();
                    logger.error("! Invalid value for --sampleCount argument: " +
                        Integer.toString(args.dataSize));
                    System.exit(-1);
                }
                continue;
        ...
        // reconfigure using our configuration file
