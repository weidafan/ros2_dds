    ...
    RTI_RoutingServiceEnvironment * env)
{
     ...
     * Trim spaces
     */
    buffer = RTI_RoutingServiceFileAdapter_trim(buffer);
    if (*buffer == 0){
        return -1;
    }
    ...
    while (token != NULL) {
        ...
        /* Remove spaces */
        field = RTI_RoutingServiceFileAdapter_trim(token);
        ...
        /* Remove spaces */
        value = RTI_RoutingServiceFileAdapter_trim(token);
        ...
        if (!RTI_RoutingServiceFileAdapter_assign(
    ...
    RTI_RoutingServiceEnvironment * env)
{
    ...
    for (i=0; i<memberCount; i++) {
        ...
        /* Print this member */
        if (!RTI_RoutingServiceFileAdapter_write_member(
