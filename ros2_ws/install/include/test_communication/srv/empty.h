// generated from rosidl_generator_c/resource/srv.h.em
// generated code does not contain a copyright notice

#ifndef TEST_COMMUNICATION__SRV__EMPTY_H_
#define TEST_COMMUNICATION__SRV__EMPTY_H_

#include "test_communication/srv/empty__request.h"
#include "test_communication/srv/empty__response.h"

#include "rosidl_generator_c/message_type_support_struct.h"

// This header provides a definition for the rosidl_service_type_support_t struct.
#include "rosidl_generator_c/service_type_support.h"

#include "rosidl_typesupport_interface/macros.h"

#include "test_communication/msg/rosidl_generator_c__visibility_control.h"

#if defined(__cplusplus)
extern "C"
{
#endif

// Forward declare the get type support functions for this type.
ROSIDL_GENERATOR_C_PUBLIC_test_communication
const rosidl_service_type_support_t *
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, test_communication, Empty)();

#if defined(__cplusplus)
}
#endif

#endif  // TEST_COMMUNICATION__SRV__EMPTY_H_
