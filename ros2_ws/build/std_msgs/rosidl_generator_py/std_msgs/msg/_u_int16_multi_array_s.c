...
{
  std_msgs__msg__UInt16MultiArray * ros_message = std_msgs__msg__UInt16MultiArray__create();
  (void)ros_message;
  ...
  if (!rosidl_generator_c__uint16__Array__init(&(ros_message->data), sizedata)) {
    PyErr_SetString(PyExc_RuntimeError, "unable to create uint16__Array ros_message");
    return NULL;
  }
  uint16_t * destdata = ros_message->data.data;
  uint16_t tmpdata;
  size_t idxdata;
  for (idxdata = 0; idxdata < sizedata; idxdata++) {
    itemdata = PySequence_Fast_GET_ITEM(seqdata, idxdata);
    assert(PyLong_Check(itemdata));
    tmpdata = (uint16_t)PyLong_AsUnsignedLong(itemdata);

    memcpy(&destdata[idxdata], &tmpdata, sizeof(uint16_t));
  }
...
void std_msgs_u_int16_multi_array__destroy_ros_message(void * raw_ros_message)
{
  ...
  std_msgs__msg__UInt16MultiArray__destroy(ros_message);
}
...
PyObject * std_msgs_u_int16_multi_array__convert_to_py(void * raw_ros_message)
{
  ...
  uint16_t * tmpmessagedatadata = ros_message->data.data;
