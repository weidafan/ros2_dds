...
{
  std_msgs__msg__UInt32MultiArray * ros_message = std_msgs__msg__UInt32MultiArray__create();
  (void)ros_message;
  ...
  assert(strncmp(
      "std_msgs.msg._u_int32_multi_array.UInt32MultiArray",
  ...
  if (!rosidl_generator_c__uint32__Array__init(&(ros_message->data), sizedata)) {
    PyErr_SetString(PyExc_RuntimeError, "unable to create uint32__Array ros_message");
    return NULL;
  }
  uint32_t * destdata = ros_message->data.data;
  uint32_t tmpdata;
  size_t idxdata;
  for (idxdata = 0; idxdata < sizedata; idxdata++) {
    itemdata = PySequence_Fast_GET_ITEM(seqdata, idxdata);
    assert(PyLong_Check(itemdata));
    tmpdata = PyLong_AsUnsignedLong(itemdata);

    memcpy(&destdata[idxdata], &tmpdata, sizeof(uint32_t));
  }
...
PyObject * std_msgs_u_int32_multi_array__convert_to_py(void * raw_ros_message)
{
  ...
  PyObject * pymessage_module = PyImport_ImportModule("std_msgs.msg._u_int32_multi_array");
  ...
  uint32_t * tmpmessagedatadata = ros_message->data.data;
