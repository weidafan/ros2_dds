...
{
  std_msgs__msg__Int64MultiArray * ros_message = std_msgs__msg__Int64MultiArray__create();
  (void)ros_message;
  ...
  assert(strncmp(
      "std_msgs.msg._int64_multi_array.Int64MultiArray",
...
PyObject * std_msgs_int64_multi_array__convert_to_py(void * raw_ros_message)
{
  ...
  PyObject * pymessage_module = PyImport_ImportModule("std_msgs.msg._int64_multi_array");
  ...
  int64_t * tmpmessagedatadata = ros_message->data.data;
  pydata = PyList_New(sizedata);
  size_t idxdata;
  for (idxdata = 0; idxdata < sizedata; idxdata++) {
    PyList_SetItem(pydata, idxdata, PyLong_FromLongLong(tmpmessagedatadata[idxdata]));
  }
  assert(PySequence_Check(pydata));
  assert(pydata != NULL);
  Py_INCREF(pydata);
