...
set(_exported_libraries "rmw")
set(_exported_library_names "")

# populate rmw_LIBRARIES
if(NOT _exported_libraries STREQUAL "")
  ...
  while(_i LESS _length)
    ...
    if("${_arg}" MATCHES "^-" AND NOT "${_arg}" MATCHES "^-[l|framework]")
      list(APPEND rmw_LIBRARIES "${_arg}")
      math(EXPR _i "${_i} + 1")
      continue()
    endif()
    ...
    if("${_arg}" MATCHES "^debug|optimized|general$")
      ...
      math(EXPR _i "${_i} + 1")
      if(_i EQUAL _length)
        message(FATAL_ERROR "Package 'rmw' passes the build configuration keyword '${_cfg}' as the last exported library")
      endif()
    ...
    if(NOT IS_ABSOLUTE "${_library}")
      ...
      if(NOT _lib)
        ...
        message(FATAL_ERROR "Package 'rmw' exports the library '${_library}' which couldn't be found")
      elseif(NOT IS_ABSOLUTE "${_lib}")
        # the found library must be an absolute path
        message(FATAL_ERROR "Package 'rmw' found the library '${_library}' at '${_lib}' which is not an absolute path")
      ...
      if(NOT EXISTS "${_library}")
        # the found library must exist
        message(WARNING "Package 'rmw' exports the library '${_library}' which doesn't exist")
      else()
        list(APPEND rmw_LIBRARIES ${_cfg} "${_library}")
      endif()
    endif()
  endwhile()
endif()
