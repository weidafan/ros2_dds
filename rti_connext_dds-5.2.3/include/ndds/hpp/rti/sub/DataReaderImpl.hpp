...
public:
    ...
    DataReaderImpl(
        ...
        const dds::topic::TopicDescription<T>& topic_description)
        : UntypedDataReader(
            detail::create_native_reader(sub, topic_description), sizeof(T)),
          subscriber_(sub),
    ...
    DataReaderImpl(
        ...
        const dds::core::status::StatusMask& mask =
            dds::core::status::StatusMask::all())
        : UntypedDataReader(
            detail::create_native_reader(sub, topic_description, &qos, listener, mask),
            sizeof(T)),
          subscriber_(sub),
          topic_description_(topic_description)
    {
    ...
    // For internal use
    DataReaderImpl(DDS_DataReader * native_reader)
        : UntypedDataReader(
            std::make_pair(DDS_DataReader_as_entity(native_reader), false),
            sizeof(T)),
          ...
          topic_description_(dds::core::null)
    {
        ...
        DDS_Subscriber * native_subscriber =
            DDS_DataReader_get_subscriber(native_reader);
        ...
        DDS_TopicDescription * native_topic_description =
            DDS_DataReader_get_topicdescription(native_reader);
    ...
    dds::sub::LoanedSamples<T> read_or_take(const SelectorState& selector_state, bool take)
    {
    ...
    uint32_t read_or_take(
       SamplesIterator& sit, const SelectorState& selector_state, bool take)
    {
        ...
        LoanedSamples<T> loaned_samples =
            read_or_take(selector_state, take);
    ...
    dds::core::InstanceHandle lookup_instance(const T& key_holder)
    {
        return DDS_DataReader_lookup_instance_untypedI(
            const_cast<DDS_DataReader*>(native_reader()), (void *) &key_holder);
    }
