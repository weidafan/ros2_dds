...
public:
    ...
    DataWriterImpl(
        const dds::pub::Publisher& pub,
        const dds::topic::Topic<T>& topic)
        : UntypedDataWriter(detail::create_native_writer(pub, topic)),
          publisher_(pub),
          topic_(topic)
    {
    }
    ...
    DataWriterImpl(
        ...
        const dds::core::status::StatusMask& mask =
            dds::core::status::StatusMask::all())
        : UntypedDataWriter(
            detail::create_native_writer(pub, topic, &qos, listener, mask)),
          publisher_(pub),
          topic_(topic)
    {
        if (listener != NULL) {
    ...
    // For internal use
    DataWriterImpl(DDS_DataWriter * native_writer)
        : UntypedDataWriter(
            std::make_pair(DDS_DataWriter_as_entity(native_writer), false)),
          ...
          topic_(dds::core::null)
    {
        ...
        DDS_Publisher * native_publisher =
            DDS_DataWriter_get_publisher(native_writer);
        ...
        DDS_Topic * native_topic =
            DDS_DataWriter_get_topic(native_writer);
...
public:
    ...
    void write(const T& sample)
    {
        DDS_ReturnCode_t retcode = DDS_DataWriter_write_untypedI(
            native_writer(), 
            (void *) &sample, 
            &dds::core::InstanceHandle::nil()->native());
