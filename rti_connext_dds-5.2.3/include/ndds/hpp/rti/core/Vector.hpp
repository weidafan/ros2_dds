...
public:
     ...
     */
    vector(const std::vector<T>& other) : Base (other.capacity())
    {
        if (other.size() != 0) {
    ...
    void resize(size_type new_size, const T& value)
    {
        ...
        if (new_size > size()) {
            ...
            destroy(get_buffer() + new_size, get_end_of_contents());
            this->set_length(new_size);
        }
    }
    ...
    {
        if (get_buffer() != NULL) {
            destroy(get_buffer(), get_end_of_contents());
            this->set_length(0);
        }
    }
...
private:
    ...
    vector(const vector& other, size_type capacity)
        : Base(other.size() > capacity ? other.size() : capacity)
    {
...
std::ostream& operator <<(std::ostream& out, const vector<_Tp>& vector)
{
    out<<"{";
    if (vector.size() > 0) {
        std::copy(vector.begin(), vector.end() - 1,
            std::ostream_iterator < _Tp > (out, ", "));
        std::copy(vector.end() - 1, vector.end(),
            std::ostream_iterator < _Tp > (out, ""));
    }
    out<<"}";
    return out;
