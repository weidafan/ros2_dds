...
public:
    ...
    UserDataImpl& value(OCTET_ITER begin, OCTET_ITER end)
    {
        value_impl(
            begin,
            end,
...
private:
    template <typename OCTET_ITER>
    void value_impl(
        OCTET_ITER begin, OCTET_ITER end, std::random_access_iterator_tag)
    {
        // Knowing the size of the range [begin, end) allows for an optimized
        // implementation.
        resize(end - begin);
        std::copy(begin, end, DDS_OctetSeq_get_contiguous_buffer(&native().value));
    }

    template <typename OCTET_ITER>
    void value_impl(
        OCTET_ITER begin, OCTET_ITER end, std::input_iterator_tag)
    {
        // Generic, less-efficient implementation for iterators that don't
        // support calculating the length of a range
        int sequence_index = 0;
        for (OCTET_ITER it = begin; it != end; ++it) {
...
public:
   ...
   // TopicData& value(const dds::core::ByteSeq& seq);
    ...
    template <typename OCTET_ITER>
    TopicDataImpl& value(OCTET_ITER begin, OCTET_ITER end)
    {
        value_impl(
            begin,
            end,
...
public:
   ...
   // GroupData& value(const dds::core::ByteSeq& seq);
    ...
    template <typename OCTET_ITER>
    GroupDataImpl& value(OCTET_ITER begin, OCTET_ITER end)
    {
        value_impl(
            begin,
            end,
...
public:
     ...
     */
    template <typename EntryIter>
    Property& set(EntryIter begin, EntryIter end, bool propagate = false)
    {
        for (; begin != end; ++begin) {
            set(*begin, propagate);
        }
    ...
    {
        set(entries.begin(), entries.end(), propagate);
    }
  #endif
...
public:
    ...
    WireProtocol& rtps_auto_id_kind(
        const rti::core::policy::WireProtocolAutoKind the_kind);
