...
public:
    ...
    {
        if (_test_count + 1 >= Tester::MAX_TEST_COUNT) {
            throw std::logic_error("reached max number of test functions");
        }

        _test_functions[_test_count++] = fnc;
    }
...
private:
    ...
    RTITestFunction _test_functions[MAX_TEST_COUNT];
    int _test_count;
