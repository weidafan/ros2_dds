...
public:
    ...
    ContentFilteredTopicImpl(
        ...
        : UntypedTopicDescription(
            detail::create_native_cft(topic, name, filter),
            topic.participant(),
            name,
            topic.type_name()),
          topic_(topic)
...
public:
     ...
     *
     * @param filter Contains the new filter expression and parameters. The
     * filter name is immutable: if filter->name() is different it will be
     * ignored.
     *
     * @details @dref_details_ContentFilteredTopic_set_filter_expression
     */
