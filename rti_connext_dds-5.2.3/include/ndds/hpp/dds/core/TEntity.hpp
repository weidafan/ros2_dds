...
public:
    ...
    // Spec error: operator=(null_type) has to be defined, even
    // though Reference already does. That's because we're defining other
    // overloaded operator= (in OMG_DDS_REF_TYPE()) and that hides the base
    // class operator=(null_type).
    //
    // If we don't define it again, expressions like the following won't work:
    // dds::core::Entity entity = ...;
    // entity = dds::core::null;
    TEntity& operator=(null_type)
    {
        dds::core::Reference<DELEGATE>::operator =(null);
        return *this;
    }
