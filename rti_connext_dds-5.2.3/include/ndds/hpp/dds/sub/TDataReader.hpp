...
public:
    ...
    public:
         ...
         * @endcode
         * 
         * @param condition The ReadCondition (or QueryCondition) to read/take with
         */
        Selector& condition(
           const dds::sub::cond::ReadCondition& condition)
        {
            impl_.condition(condition);
            return *this;
        }
        ...
        uint32_t
        read(SamplesFWIterator sfit, int32_t max_samples)
        {
        ...
        template<typename SamplesFWIterator>
        uint32_t
        take(SamplesFWIterator sfit, int32_t max_samples)
        {
...
public:
    ...
    DataReader(
        ...
        const dds::core::status::StatusMask& mask =
            ...
            dds::core::TEntity<DELEGATE<T> >(
                new DELEGATE<T>(sub, topic, qos, listener, mask))
    {
        this->delegate()->remember_reference(this->delegate());
    }
    ...
    DataReader(dds::core::construct_from_native_tag_t, DELEGATE<T>* delegate)
        : dds::core::TEntity<DELEGATE<T> >(delegate)
    {
        this->delegate()->remember_reference(this->delegate());
    }
    ...
    // Spec issue: operator=(null_type) has to be defined explicitly again
    DataReader& operator=(dds::core::null_type)
    {
   ...
   * If there's no listener it returns NULL.
     ...
     *
     * @param qos The DataReaderQos to be set to.
