...
//
// ATTENTION: this file contains modifications by RTI 
//            to support DKMs (Kernel mode). Look for "Modified by RTI"
//
...
// --------------------------------
#define RTIBOOST_PLATFORM "vxWorks"
...
#define RTIBOOST_LOCALE_WITH_ICU
...
//#define RTIBOOST_NO_DEDUCED_TYPENAME // Commented this out, boost's test gives an errorneous result!
...
#if defined(__RTP__) || defined(_WRS_KERNEL) // Modified by RTI
#  include <time.h>
#  include <sysLib.h>
#endif
...
inline int truncate(const char *p, off_t l){
 ...
 #else // Modified by RTI: kernel requires a third parameter, the file mode
  int fd = open(p, O_WRONLY, 0644); // Modified by RTI
 #endif
  if (fd == -1){
    errno = EACCES;
    return -1;
