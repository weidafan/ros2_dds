...
/**
* A sequence of ThroughputCommand instances.
*/
public final class ThroughputCommandSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThroughputCommandSeq() {
        super(ThroughputCommand.class);
    }

    public ThroughputCommandSeq (int initialMaximum) {
        super(ThroughputCommand.class, initialMaximum);
    }

    public ThroughputCommandSeq (Collection elements) {
        super(ThroughputCommand.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThroughputCommand.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThroughputCommand objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThroughputCommand.create());
