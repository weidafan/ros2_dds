...
public class HelloPublisher {
    ...
    private StringDataWriter     _theDataWriter;
    ...
    public HelloPublisher(DomainParticipant participant,
        ...
        _theDataWriter = (StringDataWriter)
    ...
    void writeSamples() {
        ...
        for (int count = 0;; ++count) {
            // Fill up the first 9 character of the payload string by writing
            // the count number (converted to string) in the StringBuffer
            //
            ...
            for (int l = 0; l < countString.length(); ++l) {
                instanceBuffer.setCharAt(l, countString.charAt(l));
            }
            try {
                String bufferToWrite = instanceBuffer.toString();
                _theDataWriter.write(bufferToWrite, InstanceHandle_t.HANDLE_NIL);
