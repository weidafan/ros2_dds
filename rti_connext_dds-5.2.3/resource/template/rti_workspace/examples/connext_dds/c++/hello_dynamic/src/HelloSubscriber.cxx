...
private:
    ...
    DDS_Long _theLastSampleId;  // Sample ID of the last received sample
    size_t _thePayloadSize;     // Size of data payload (set once)
...
public:    
     ...
     */
    void processData(DDS_DynamicData &instance) {
        /* Since the DynamicData contains any generic data type, you can
         * query the type and the details just like this:
         *
         *
        {
            int numFields = instance.get_member_count();
            // Access all the fields
            for (int i = 0; i < numFields; ++i) {
                DDS_DynamicDataMemberInfo info;
                if (instance.get_member_info_by_index(info, i) != 
                        DDS_RETCODE_OK) {
                    std::cerr << "! failed to get member info" << std::endl;
                    break;
                }
                switch(info.member_kind) {
                    case DDS_TK_LONG: {
                        DDS_Long val;
                        instance.get_long(val, info.member_name, 
                                          info.member_id);
                        std::cout << "Long member: " << val << std::endl;
                        break;
                    }
                    case DDS_TK_STRING: {
                        DDS_Char *string = NULL;
                        instance.get_string(string, NULL, info.member_name, 
                            info.member_id);
                        std::cout << "String member: " << string << std::endl;
                        break;
                    }
                    default:
                        std::cout << "Type=" << info.member_kind << ", size="
                                  << info.element_count << std::endl;
                }
            } // Foreach field
        }
        */
     
        /* In our case, the data we expect is something like this:
         * 
         * struct HelloWorld {
         *     string<64>            prefix;
         *     long                  sampleId;
         ...
         * };
         */
        instance.get_long(_theLastSampleId, 
                        "sampleId", 
        ...
        if (_thePayloadSize == 0) {
            ...
            payload.ensure_length(HELLODDS_MAX_PAYLOAD_SIZE, HELLODDS_MAX_PAYLOAD_SIZE);
            instance.get_octet_seq(payload,
                        "payload", 
                        DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED);
            _thePayloadSize = payload.length();
            // std::cout << "Payload size=" << _thePayloadSize << std::endl;
        }
    }
...
public:
    ...
    void on_data_available(DDSDataReader* reader) {
         ...
         */
        DDSDynamicDataReader *helloReader = DDSDynamicDataReader::narrow(reader);
        if (helloReader == NULL) {
