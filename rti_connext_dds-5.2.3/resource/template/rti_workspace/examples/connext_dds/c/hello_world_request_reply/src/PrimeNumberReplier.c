    ...
    const struct DDS_SampleIdentity_t * request_id)
{
    ...
    if (reply == NULL) {
        printf("Create reply data error\n");
    }
...
int replier_main(int domain_id)
{
    ...
    if (request == NULL) {
        printf("Create request data error\n");
        replier_shutdown(participant, replier, request);
        return -1;
    }
