...
typedef struct CallbackData_t {
    DDS_Boolean verbose;
    DDS_Long sample_id;         /* Sample ID of the last received sample     */
...
static void process_data(
     ...
     *
    {
        int numFields = DDS_DynamicData_get_member_count(instance);
        int i;
        // Access all the fields
        for (i = 0; i < numFields; ++i) {
            struct DDS_DynamicDataMemberInfo info;
            if (DDS_DynamicData_get_member_info_by_index(instance, &info, i)
                    != DDS_RETCODE_OK) {
                fprintf(stderr, "! failed to get member info\n");
                break;
            }
            switch(info.member_kind) {
                case DDS_TK_LONG: {
                    DDS_Long val;
                    DDS_DynamicData_get_long(instance, 
                        &val, 
                        info.member_name, 
                        info.member_id);
                    printf("Long member: %d\n", val);
                    break;
                }
                case DDS_TK_STRING: {
                    DDS_Char *string = NULL;
                    DDS_DynamicData_get_string(instance,
                        &string, 
                        NULL, 
                        info.member_name, 
                        info.member_id);
                ...
                default:
                    printf("Type=%d, size=%d\n", 
                        info.member_kind, info.element_count);
            }
        } // Foreach field
    }
    */
    ...
    if (me->payload_size == 0) {
        ...
        DDS_OctetSeq_ensure_length(&payload,
                    HELLODDS_MAX_PAYLOAD_SIZE, 
                    HELLODDS_MAX_PAYLOAD_SIZE);
        DDS_DynamicData_get_octet_seq(instance,
                    &payload,
                    "payload", 
                    DDS_DYNAMIC_DATA_MEMBER_ID_UNSPECIFIED);
        me->payload_size = DDS_OctetSeq_get_length(&payload);
...
static void on_data_available(
                        ...
                        DDS_DataReader* reader) {
    CallbackData *me                   = (CallbackData *)listener_data;
    DDS_DynamicDataReader *helloReader = NULL;
     ...
     */
    helloReader = DDS_DynamicDataReader_narrow(reader);
    if (helloReader == NULL) {
    ...
    for (i = 0; i < DDS_DynamicDataSeq_get_length(&data_seq); ++i) {
        if (DDS_SampleInfoSeq_get_reference(&info_seq, i)->valid_data) {
            /* Process the data */
            process_data(me, DDS_DynamicDataSeq_get_reference(&data_seq, i));
        }
    }
     ...
     */
    rc = DDS_DynamicDataReader_return_loan(
                        helloReader,
                        &data_seq, 
                        &info_seq);
...
DDS_Boolean start_subscriber(DDS_DomainParticipant *participant,
    ...
    callback_data.sample_rcvd_max = sampleCount;
    ...
    for (;;) {
        ...
        if (sampleCount != 0 && (callback_data.sample_rcvd >= sampleCount)) {
