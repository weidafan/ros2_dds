...
typedef struct CallbackData_t {
    DDS_Boolean verbose;
    long sampleId;             /* Sample ID of the last received sample     */
...
static void process_data(
                        CallbackData *me,
                        const HelloWorld * const instance) {
    
    /* If you want to verify that the payload is correct, uncomment the
     * following section:
     *
    {
        DDS_Octet *payloadBuffer = NULL;
        int pos;
        payloadBuffer = DDS_OctetSeq_get_contiguous_buffer(&instance->payload);
        for (pos = 0; pos < DDS_OctetSeq_get_length(&instance->payload);
                        ++pos) {
            if (payloadBuffer[pos] != (pos & 0xff)) {
                fprintf(stderr, "! data is not the same\n");
                return;
            }
        }
    }
    ...
    if (me->payload_size == 0) {
        ...
        me->payload_size = DDS_OctetSeq_get_length(&instance->payload);
    }
    me->sampleId = instance->sampleId;
    ++(me->sample_rcvd);
}
...
static void on_data_available(
                        ...
                        DDS_DataReader* reader) {
    CallbackData *me                  = (CallbackData *)listener_data;
    HelloWorldDataReader *helloReader = NULL;
     ...
     */
    helloReader = HelloWorldDataReader_narrow(reader);
    if (helloReader == NULL) {
    ...
    for (i = 0; i < HelloWorldSeq_get_length(&data_seq); ++i) {
        if (DDS_SampleInfoSeq_get_reference(&info_seq, i)->valid_data) {
            /* Process the data */
            process_data(me, HelloWorldSeq_get_reference(&data_seq, i));            
        }
    }
     ...
     */
    rc = HelloWorldDataReader_return_loan(
                        helloReader,
                        &data_seq, 
                        &info_seq);
...
DDS_Boolean start_subscriber(DDS_DomainParticipant *participant,
    ...
    long last_sampleId = 0;    /* Copy of the last sampleId */
    long last_sample_lost = 0;  /* Copy of the last sample_lost */
    long prev_sampleId = 0;    /* ID of sample lost on prev. iteration */
     ...
     * use DDS_DomainParticipant_get_default_subscriber_qos() to 
     ...
     *     $NDDSHOME/doc/html/api_c/group__DDSSubscriberModule.html
     */
    if (verbose) {
        printf("Creating the subscriber...\n");
    }
    ...
    if (subscriber == NULL) {
        fprintf(stderr, "! Unable to create DDS Subscriber\n");
    ...
    data_reader = DDS_Subscriber_create_datareader(
                        subscriber,
    ...
    for (;;) {
        ...
        /* Stop after sample_count has been received */
        if (sample_count != 0 && (callback_data.sample_rcvd >= sample_count)) {
            printf("\nReceived %ld samples.\n", callback_data.sample_rcvd);
            break;
        }

        /* If the last sampleId received is < than the older one received,
         ...
         */
        if (callback_data.sampleId < last_sampleId) {
        ...
        last_sampleId   = callback_data.sampleId;
        ...
        if (stat_first_sequence_id == 0) {
            stat_first_sequence_id = last_sampleId;
            start_time = time(NULL);
            /* Don't consider this iteration in the statistics, as it is
             * not meaningful.
             */
            prev_sampleId = last_sampleId;
        ...
        stat_total_samples = last_sampleId - stat_first_sequence_id 
                        - last_sample_lost;
        stat_total_sample_per_sec = (float)stat_total_samples / (float)stat_delta_time_sec;
        stat_current_sample_per_sec = (float)(last_sampleId - prev_sampleId) 
                        / POLL_PERIOD_SEC;
